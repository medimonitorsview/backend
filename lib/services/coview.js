...
const mapName = (name, value, deviceCategory) => {
    try {
        ...
        const mappedName = enriched.map(n => {
            ...
            if (newName) {
                const value = isNumeric(n.value)?parseFloat(n.value):n.value
    ...
    try {
        ...
        if (filtered.length) {
            const transformed = filtered.map(m => {
                ...
                const initial = { timestamp: now.valueOf(), MachineId: m.monitorId, name: `${monitorInfo.roomId}:${monitorInfo.patientId}` };
